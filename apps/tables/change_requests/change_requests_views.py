...
def create_change_requests_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'ChangeRequests', 'CHANGE_REQUESTS')
    return redirect(request.META.get('HTTP_REFERER'))

def create_change_requests_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'CHANGE_REQUESTS')
        return redirect(request.META.get('HTTP_REFERER'))

def create_change_requests_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'CHANGE_REQUESTS')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_change_requests_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_change_requests_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_change_requests_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_change_requests_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def change_requests(request):
    db_field_names = [field.name for field in ChangeRequests._meta.get_fields() if not field.is_relation]
    try:
    ...
    selected_rows = SelectedRows.objects.filter(
        ...
        model_choice=ModelChoices.CHANGE_REQUESTS
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'changerequests')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS, field_type=FieldType.FLOAT)
    ...
    base_queryset = ChangeRequests.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_changerequests')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_changerequests')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_changerequests')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_changerequests')
    ...
    read_only_fields = ('fts', 'hash_data', 'ID', 'loader_instance', 'json_data',)
    pre_column = ('fts', 'hash_data', 'ID', 'loader_instance', 'json_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('ChangeRequests', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.CHANGE_REQUESTS
    ).values())
    ...
    context = {
        ...
        'total_items': ChangeRequests.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': ChangeRequests.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'Change Requests',
        'int_filter_instance': int_filter_instance,
        'int_fields': ChangeRequests.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': ChangeRequests.float_fields,
        'encrypted_fields': ChangeRequests.encrypted_fields,
    ...
    return render(request, 'apps/change_requests.html', context)


@login_required(login_url='/users/signin/')
def create_change_requests(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in ChangeRequests.date_fields_to_convert:
                if attribute in ChangeRequests.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in ChangeRequests.float_fields:
        ...
        ChangeRequests.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_change_requests(request, id):
    software = ChangeRequests.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_change_requests(request, id):
    software = ChangeRequests.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in ChangeRequests.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.CHANGE_REQUESTS)
        ...
        response['Content-Disposition'] = 'attachment; filename="change_requests.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = ChangeRequests.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in ChangeRequests.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="change_requests.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="change_requests.pdf"'
        ...
        softwares = ChangeRequests.objects.all()
