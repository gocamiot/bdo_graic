...
def create_unified_access_management_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'UnifiedAccessManagement', 'UNIFIED_ACCESS_MANAGEMENT')
    return redirect(request.META.get('HTTP_REFERER'))

def create_unified_access_management_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'UNIFIED_ACCESS_MANAGEMENT')
        return redirect(request.META.get('HTTP_REFERER'))

def create_unified_access_management_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'UNIFIED_ACCESS_MANAGEMENT')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_unified_access_management_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_unified_access_management_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_unified_access_management_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_unified_access_management_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def unified_access_management(request):
    db_field_names = [field.name for field in UnifiedAccessManagement._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='UnifiedAccessManagement',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='unifiedaccessmanagement', 
        model_choice=ModelChoices.UNIFIED_ACCESS_MANAGEMENT
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'unifiedaccessmanagement')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT, field_type=FieldType.FLOAT)
    ...
    base_queryset = UnifiedAccessManagement.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_unifiedaccessmanagement')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_unifiedaccessmanagement')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_unifiedaccessmanagement')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_unifiedaccessmanagement')
    ...
    try:
        ...
        return redirect('unified_access_management')
    except EmptyPage:
        return redirect('unified_access_management') 

    read_only_fields = ('fts', 'ID', 'loader_instance', 'hash_data', 'json_data',)
    pre_column = ('fts', 'ID', 'loader_instance', 'hash_data', 'json_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('UnifiedAccessManagement', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT
    ).values())
    ...
    context = {
        'segment'  : 'unified_access_management',
        'parent'   : 'uam',
        'softwares' : softwares,
        ...
        'total_items': UnifiedAccessManagement.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': UnifiedAccessManagement.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'Unified Access Management',
        'int_filter_instance': int_filter_instance,
        'int_fields': UnifiedAccessManagement.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': UnifiedAccessManagement.float_fields,
        'encrypted_fields': UnifiedAccessManagement.encrypted_fields,
    ...
    return render(request, 'apps/unified_access_management.html', context)


@login_required(login_url='/users/signin/')
def create_unified_access_management(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in UnifiedAccessManagement.date_fields_to_convert:
                if attribute in UnifiedAccessManagement.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in UnifiedAccessManagement.float_fields:
        ...
        UnifiedAccessManagement.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_unified_access_management(request, id):
    software = UnifiedAccessManagement.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_unified_access_management(request, id):
    software = UnifiedAccessManagement.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in UnifiedAccessManagement.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.UNIFIED_ACCESS_MANAGEMENT)
        ...
        response['Content-Disposition'] = 'attachment; filename="unified_access_management.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = UnifiedAccessManagement.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in UnifiedAccessManagement.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="unified_access_management.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="unified_access_management.pdf"'
        ...
        softwares = UnifiedAccessManagement.objects.all()
