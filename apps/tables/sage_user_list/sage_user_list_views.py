...
def create_sage_user_list_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'SAGEUserList', 'SAGE_USER_LIST')
    return redirect(request.META.get('HTTP_REFERER'))

def create_sage_user_list_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'SAGE_USER_LIST')
        return redirect(request.META.get('HTTP_REFERER'))

def create_sage_user_list_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'SAGE_USER_LIST')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_sage_user_list_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_sage_user_list_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_sage_user_list_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_sage_user_list_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def sage_user_list(request):
    db_field_names = [field.name for field in SAGEUserList._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='SAGEUserList',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='sageuserlist', 
        model_choice=ModelChoices.SAGE_USER_LIST
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'sageuserlist')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST, field_type=FieldType.FLOAT)
    ...
    base_queryset = SAGEUserList.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_sageuserlist')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_sageuserlist')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_sageuserlist')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_sageuserlist')
    ...
    try:
        ...
        return redirect('sage_user_list')
    except EmptyPage:
        return redirect('sage_user_list') 

    read_only_fields = ('fts', 'ID', 'json_data', 'hash_data', 'loader_instance',)
    pre_column = ('fts', 'ID', 'json_data', 'hash_data', 'loader_instance',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('SAGEUserList', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.SAGE_USER_LIST
    ).values())
    ...
    context = {
        ...
        'total_items': SAGEUserList.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': SAGEUserList.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'SAGE User List',
        'int_filter_instance': int_filter_instance,
        'int_fields': SAGEUserList.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': SAGEUserList.float_fields,
        'encrypted_fields': SAGEUserList.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='sage_user_list').order_by('created_at'),
        'join_model_instance': SAGEUserList.join_model_instance if hasattr( SAGEUserList, 'join_model_instance') else None,
    ...
    return render(request, 'apps/sage_user_list.html', context)


@login_required(login_url='/users/signin/')
def create_sage_user_list(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in SAGEUserList.date_fields_to_convert:
                if attribute in SAGEUserList.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in SAGEUserList.float_fields:
        ...
        SAGEUserList.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_sage_user_list(request, id):
    software = SAGEUserList.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_sage_user_list(request, id):
    software = SAGEUserList.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in SAGEUserList.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.SAGE_USER_LIST)
        ...
        response['Content-Disposition'] = 'attachment; filename="sage_user_list.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = SAGEUserList.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in SAGEUserList.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="sage_user_list.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="sage_user_list.pdf"'
        ...
        softwares = SAGEUserList.objects.all()
