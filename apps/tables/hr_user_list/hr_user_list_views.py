...
def create_hr_user_list_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'HRUserList', 'HR_USER_LIST')
    return redirect(request.META.get('HTTP_REFERER'))

def create_hr_user_list_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'HR_USER_LIST')
        return redirect(request.META.get('HTTP_REFERER'))

def create_hr_user_list_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'HR_USER_LIST')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_hr_user_list_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_user_list_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_user_list_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_user_list_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def hr_user_list(request):
    db_field_names = [field.name for field in HRUserList._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='HRUserList',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='hruserlist', 
        model_choice=ModelChoices.HR_USER_LIST
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'hruserlist')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST, field_type=FieldType.FLOAT)
    ...
    base_queryset = HRUserList.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_hruserlist')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_hruserlist')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_hruserlist')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_hruserlist')
    ...
    read_only_fields = ('json_data', 'ID', 'loader_instance', 'fts', 'hash_data',)
    pre_column = ('json_data', 'ID', 'loader_instance', 'fts', 'hash_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('HRUserList', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.HR_USER_LIST
    ).values())
    ...
    context = {
        ...
        'total_items': HRUserList.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': HRUserList.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'HR User List',
        'int_filter_instance': int_filter_instance,
        'int_fields': HRUserList.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': HRUserList.float_fields,
        'encrypted_fields': HRUserList.encrypted_fields,
    ...
    return render(request, 'apps/hr_user_list.html', context)


@login_required(login_url='/users/signin/')
def create_hr_user_list(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in HRUserList.date_fields_to_convert:
                if attribute in HRUserList.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in HRUserList.float_fields:
        ...
        HRUserList.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_hr_user_list(request, id):
    software = HRUserList.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_hr_user_list(request, id):
    software = HRUserList.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in HRUserList.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.HR_USER_LIST)
        ...
        response['Content-Disposition'] = 'attachment; filename="hr_user_list.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = HRUserList.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in HRUserList.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="hr_user_list.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="hr_user_list.pdf"'
        ...
        softwares = HRUserList.objects.all()
