...
def create_control_ad_user_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'ControlADUser', 'CONTROL_AD_USER')
    return redirect(request.META.get('HTTP_REFERER'))

def create_control_ad_user_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'CONTROL_AD_USER')
        return redirect(request.META.get('HTTP_REFERER'))

def create_control_ad_user_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'CONTROL_AD_USER')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_control_ad_user_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_control_ad_user_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_control_ad_user_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_control_ad_user_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def control_ad_user(request):
    db_field_names = [field.name for field in ControlADUser._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='ControlADUser',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='controladuser', 
        model_choice=ModelChoices.CONTROL_AD_USER
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'controladuser')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER, field_type=FieldType.FLOAT)
    ...
    base_queryset = ControlADUser.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_controladuser')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_controladuser')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_controladuser')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_controladuser')
    ...
    try:
        ...
        return redirect('control_ad_user')
    except EmptyPage:
        return redirect('control_ad_user') 

    read_only_fields = ('loader_instance', 'ID', 'hash_data', 'fts', 'json_data',)
    pre_column = ('loader_instance', 'ID', 'hash_data', 'fts', 'json_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('ControlADUser', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.CONTROL_AD_USER
    ).values())
    ...
    context = {
        ...
        'total_items': ControlADUser.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': ControlADUser.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'Control AD User',
        'int_filter_instance': int_filter_instance,
        'int_fields': ControlADUser.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': ControlADUser.float_fields,
        'encrypted_fields': ControlADUser.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='control_ad_user').order_by('created_at'),
        'join_model_instance': ControlADUser.join_model_instance if hasattr( ControlADUser, 'join_model_instance') else None,
    ...
    return render(request, 'apps/control_ad_user.html', context)


@login_required(login_url='/users/signin/')
def create_control_ad_user(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in ControlADUser.date_fields_to_convert:
                if attribute in ControlADUser.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in ControlADUser.float_fields:
        ...
        ControlADUser.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_control_ad_user(request, id):
    software = ControlADUser.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_control_ad_user(request, id):
    software = ControlADUser.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in ControlADUser.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.CONTROL_AD_USER)
        ...
        response['Content-Disposition'] = 'attachment; filename="control_ad_user.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = ControlADUser.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in ControlADUser.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="control_ad_user.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="control_ad_user.pdf"'
        ...
        softwares = ControlADUser.objects.all()
